...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  // Admin Sub-Tab
  const [adminTab, setAdminTab] = useState<'overview' | 'orders' | 'products' | 'photos' | 'page_content' | 'tahfidz_profile' | 'tahfidz_santri' | 'tahfidz_guru' | 'tahfidz_kegiatan' | 'kios_sedekah' | 'styling' | 'store_info' | 'pages' | 'database'>('overview');
  ...
  const [guruForm, setGuruForm] = useState<Omit<Guru, 'id'>>({
    name: '',
    gender: 'L',
    jabatan: '',
    spesialisasi: '',
  ...
  const handleSaveGuru = (e: React.FormEvent) => {
    e.preventDefault();
    if (!guruForm.name.trim() || !guruForm.jabatan.trim()) {
      alert('Nama Guru dan Jabatan wajib diisi!');
      return;
    }
    ...
    if (editingGuru) {
      ...
      db.updateGuru(updated);
      showToast(`Data guru ${guruForm.name} berhasil diperbarui!`);
    } else {
      db.addGuru(guruForm);
      showToast(`Guru baru ${guruForm.name} berhasil ditambahkan!`);
    }

    resetGuruForm();
  };

  const handleStartEditGuru = (guru: Guru) => {
    setEditingGuru(guru);
    setGuruForm({
      name: guru.name,
      gender: guru.gender,
  ...
  const [isUploadingGuruPhoto, setIsUploadingGuruPhoto] = useState(false);
  const handleGuruPhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    setIsUploadingGuruPhoto(true);
    try {
      const compressed = await compressImageFile(file, { maxDimension: 500, maxSizeKB: 70 });
      setGuruForm((prev) => ({ ...prev, photoUrl: compressed }));
      showToast('Foto guru berhasil diunggah!');
    } catch (err: any) {
      alert(err?.message || 'Gagal memproses foto.');
    } finally {
      setIsUploadingGuruPhoto(false);
      e.target.value = '';
    }
  };
  ...
  return (
    <div className="py-8 space-y-6 pb-20">
      ...
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1 border-b border-neutral-200">
        ...
        <button
          onClick={() => setAdminTab('tahfidz_guru')}
          className={`px-4 py-2.5 rounded-xl font-bold text-xs flex items-center gap-2 whitespace-nowrap transition-all cursor-pointer ${
            adminTab === 'tahfidz_guru'
              ? 'bg-emerald-900 text-white shadow-xs'
              : 'bg-emerald-50 text-emerald-900 hover:bg-emerald-100 border border-emerald-200'
          }`}
        >
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="bg-white rounded-3xl p-6 border border-neutral-200 shadow-xs space-y-4">
            <div className="flex items-center justify-between border-b border-neutral-100 pb-3">
              <h3 className="font-bold text-base text-neutral-900 flex items-center gap-2">
                <UserPlus className="w-5 h-5 text-emerald-700" />
                <span>{editingGuru ? `Edit Data Guru (${editingGuru.name})` : 'Tambah Guru / Pengajar Baru'}</span>
              </h3>
              {editingGuru && (
                <button
                  type="button"
            ...
            <form onSubmit={handleSaveGuru} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-neutral-700 mb-1">Nama Lengkap Guru *</label>
                  <input
                    type="text"
                    required
                    value={guruForm.name}
                    onChange={(e) => setGuruForm({ ...guruForm, name: e.target.value })}
                    placeholder="Ustadz H. Ahmad Fauzi"
                    className="w-full px-3.5 py-2.5 rounded-xl border border-neutral-300 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Pengajar Tahfidz Putra"
                    className="w-full px-3.5 py-2.5 rounded-xl border border-neutral-300 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-neutral-700 mb-1">Spesialisasi</label>
                  <input
                    type="text"
                    value={guruForm.spesialisasi}
                    onChange={(e) => setGuruForm({ ...guruForm, spesialisasi: e.target.value })}
                    placeholder="Hafal 30 Juz, Sanad Qira'at"
                    className="w-full px-3.5 py-2.5 rounded-xl border border-neutral-300 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-neutral-700 mb-1">No HP / WA</label>
                  <input
                    type="text"
                    value={guruForm.phone}
                    onChange={(e) => setGuruForm({ ...guruForm, phone: e.target.value })}
                    placeholder="08123456789"
                    className="w-full px-3.5 py-2.5 rounded-xl border border-neutral-300 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                  />
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-neutral-700 mb-1">Tanggal Bergabung</label>
                  <input
                    type="date"
                    value={guruForm.joinedDate}
                    onChange={(e) => setGuruForm({ ...guruForm, joinedDate: e.target.value })}
                    className="w-full px-3.5 py-2.5 rounded-xl border border-neutral-300 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-neutral-700 mb-1">Bio Singkat</label>
                <textarea
                  value={guruForm.bio}
                  onChange={(e) => setGuruForm({ ...guruForm, bio: e.target.value })}
                  rows={2}
                  placeholder="Deskripsi singkat pengalaman atau tugas mengajar..."
                  className="w-full px-3.5 py-2.5 rounded-xl border border-neutral-300 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/30 resize-none"
                />
              </div>

              {/* Photo Upload for Guru */}
              <div className="p-4 bg-neutral-50 rounded-2xl border border-neutral-200 flex flex-col sm:flex-row items-center gap-4">
                <div className="w-16 h-16 rounded-xl bg-neutral-200 border border-neutral-300 shrink-0 overflow-hidden relative group">
                  {guruForm.photoUrl ? (
                    <>
                      <img src={guruForm.photoUrl} alt="Guru Preview" className="w-full h-full object-cover" />
                      <button
                        type="button"
                        onClick={() => setGuruForm({ ...guruForm, photoUrl: '' })}
                        title="Hapus Foto"
                        className="absolute -top-1.5 -right-1.5 p-1 bg-rose-600 hover:bg-rose-700 text-white rounded-full shadow-md cursor-pointer transition-colors"
                      >
                        <X className="w-3 h-3" />
                ...
                <div className="space-y-1.5 flex-1 text-center sm:text-left">
                  <label className="block text-xs font-bold text-neutral-700">Foto Profil Guru</label>
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                    <label className="px-3.5 py-2 bg-emerald-800 text-white font-bold rounded-xl text-xs flex items-center gap-1.5 cursor-pointer hover:bg-emerald-900 transition-colors">
                      <Upload className="w-3.5 h-3.5" />
                      <span>{isUploadingGuruPhoto ? 'Memproses...' : 'Unggah Foto dari HP / Komputer'}</span>
                      <input type="file" accept="image/*" onChange={handleGuruPhotoUpload} disabled={isUploadingGuruPhoto} className="hidden" />
                    </label>
                  </div>
                </div>
              </div>
              ...
              >
                <Plus className="w-4 h-4" />
                <span>{editingGuru ? 'Simpan Perubahan Guru' : 'Tambah Guru Ke Database'}</span>
              </button>
            </form>
          </div>
          ...
          <div className="bg-white rounded-3xl p-6 border border-neutral-200 shadow-xs space-y-4">
            <h3 className="font-bold text-base text-neutral-900 flex items-center gap-2">
              <UserCheck className="w-5 h-5 text-emerald-700" /> Daftar Seluruh Guru ({guruList.length})
            </h3>
                ...
                <tbody className="divide-y divide-neutral-100">
                  {guruList.map((guru) => (
                    <tr key={guru.id} className="hover:bg-neutral-50/80 transition-colors">
                      <td className="p-3">
                        <img src={guru.photoUrl || 'https://images.unsplash.com/photo-1544717305-2782549b5136'} alt={guru.name} className="w-10 h-10 rounded-xl object-cover border" />
                      </td>
                      <td className="p-3 font-bold text-neutral-900">{guru.name} ({guru.gender})</td>
                      <td className="p-3">{guru.jabatan}</td>
                      <td className="p-3 text-neutral-500">{guru.spesialisasi || '-'}</td>
                      <td className="p-3">
                        <span className={`px-2 py-0.5 rounded-md font-bold text-[10px] ${
                          guru.status === 'Aktif' ? 'bg-emerald-100 text-emerald-900' : 'bg-neutral-200 text-neutral-700'
                        }`}>
                          {guru.status}
                        </span>
                      </td>
                      <td className="p-3">{guru.phone || '-'}</td>
                      <td className="p-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() => handleStartEditGuru(guru)}
                            className="p-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-lg cursor-pointer"
                          ...
                          <button
                            onClick={() => handleDeleteGuru(guru.id, guru.name)}
                            className="p-1.5 bg-rose-50 text-rose-700 hover:bg-rose-100 rounded-lg cursor-pointer"
                            title="Hapus Guru"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
